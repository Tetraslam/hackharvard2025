...
function detectMeteor(landmarks: LM[]): GestureDetection {
    ...
    const bothArmsVeryHigh =
        leftWrist.y < leftShoulder.y - 0.15 && rightWrist.y < rightShoulder.y - 0.15;
    const armsAboveNose = leftWrist.y < nose.y - 0.1 && rightWrist.y < nose.y - 0.1;
    const elbowsHigh =
...
function detectGesture(
    ...
    if ((leftWrist.visibility ?? 1) < 0.5 || (rightWrist.visibility ?? 1) < 0.5) {
        return { move: "none", confidence: 0, isDetected: false };
    }
    const gestures = [
					...
					const drawingUtils = new DrawingUtils(ctx);
						...
						// Draw pose skeleton - pass normalized landmarks directly
							...
							const gesture = detectGesture(
								poseResults.landmarks[0] as LM[],
								handResults.landmarks as LM[][],
