...
import { AnimationLayer } from "@/fx/AnimationLayer";
		...
		const syncLocalDisplay = () => {
			...
			if (sourceVideo && sourceVideo.srcObject && displayVideo) {
				if (displayVideo.srcObject !== sourceVideo.srcObject) {
					...
					(displayVideo as HTMLVideoElement).play().catch(() => {});
	...
	useEffect(() => {
		...
		const setupMediaAndCreateOffer = async () => {
			...
			while (attempts < maxAttempts) {
				...
				console.log(
					`Attempt ${attempts + 1}: Video not ready yet (readyState: ${videoEl?.readyState})`,
				);
				attempts++;
